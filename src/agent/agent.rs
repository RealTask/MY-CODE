...
impl Agent {
    ...
    #[inline]
    pub fn file_system_tools(&self) -> &FileSystemTools {
    ...
    pub async fn run(&mut self, mut task: Task) -> Result<String> {
        ...
        self.event_bus.publish(AgentEvent::Started {
        ...
        self.event_bus.publish(AgentEvent::PlanCreated { steps: plan.len() });
                ...
                Err(e) => {
                    ...
                    match self.debugger.debug(&e.to_string(), &task.description).await {
                        Ok(debug_result) => {
                            ...
                            results.push(format!(
                                "Failed: {} - Debug: {debug_result}",
                                step.description
                            ));
                        }
                        Err(debug_err) => {
                            warn!("Debug also failed: {debug_err}");
                            results.push(format!("Failed: {} - Error: {e}", step.description));
                        }
                    }
                }
            }
        ...
        self.event_bus.publish(AgentEvent::Finished { success });

        info!(
            "Task completed: {} (success={}, errors={})",
            task.description, success, self.state.error_count
        );
    ...
    pub async fn chat(&mut self, messages: Vec<Message>) -> Result<Message> {
        ...
        match self
            .provider
            .chat(&messages, &model, None, None, None, None)
            .await
        {
            Ok(response) => {
    ...
    pub async fn chat_with_tools(
        ...
        match self
            .provider
            .chat(&messages, &model, None, tools, None, None)
            .await
        {
            Ok(response) => {
                debug!(
                ...
                if !response.tool_calls.is_empty() {
                    message = message.with_tool_calls(response.tool_calls);
                }
                Ok(message)
            }
            Err(e) => {
    ...
    pub async fn stream_chat(
        ...
        match self
            .provider
            .stream(&messages, &model, None, None, None, None)
            .await
        {
            Ok(stream) => {
...
mod tests {
    ...
    fn test_agent_creation() {
        ...
        let agent = Agent::new(Box::new(NullProvider::new()), file_system, event_bus);
        assert!(!agent.is_busy());
        assert_eq!(agent.error_count(), 0);
        assert_eq!(agent.provider.name(), "null");
    }
    ...
    #[tokio::test]
    async fn test_agent_run_and_chat() {
        let mut agent = Agent::new(
            Box::new(NullProvider::new().with_reply("ok")),
            FileSystemTools::new(),
            EventBus::new(),
        );
        let output = agent.run(Task::new("do something")).await.unwrap();
        assert!(output.contains("Executed") || output.contains("Review"));
